...
labels = c(EDU_MAE = "Mother's education", EDU_PAI = "Father's education", RENDA_PERCAPITA = "Income (per capita)",
            TP_COR_RACA_1.0 = "White students", IN_BIBLIOTECA = "School library", DIVERSIDADE = "Faculty exclusivity", 
            IN_ATIV_COMPLEMENTAR = "Complementary activity", IN_EQUIP_PARABOLICA = "Satellite Dish",
            IN_FORM_DOCENTE = "Faculty training", IN_LABORATORIO_CIENCIAS = "Science lab", 
            ...
            NU_CIENCIAS_HUMANAS = "Humanitis faculty", NU_LINGUAGENS_CODIGOS = "Languages faculty", 
...
p1<-df%>%filter(name %in% teacher_features)%>%
  select(name, mua,mem, ale_0,year)%>%mutate(grupo = "Teacher's Features")
p2<-df%>%filter(name %in% school_features)%>%
  select(name, mua,mem, ale_0,year)%>%mutate(grupo = "School's Features")
p3<-df%>%filter(name %in% student_features)%>%
  select(name, mua, mem,ale_0,year)%>%mutate(grupo = "Student's Features")
p4<-df%>%filter(name %in% non_actionable_features)%>%
  select(name, mua,mem,ale_0, year)%>%mutate(grupo = "Non-actionable features")
...
histo%>%ggplot(aes(x=grupo, y =mua, color=grupo))+geom_boxplot()+geom_jitter()+
  ...
  xlab("")+ylab(" Uncentered ALE at Specific Value (average)")+theme(legend.title = element_blank(),
                                                  #text=element_text(size=12),
                                                  axis.text.x = element_text(color = 'gray15'),
                                                  axis.text.y = element_text(color = 'gray15'))
...
track_importance<-histo%>%ungroup()%>%group_by(year)%>%slice_max(abs(mua), n=10)
ggplot(data=track_importance, aes(x=as.character(year), group=grupo, fill=grupo)) +
  geom_density(adjust=0.5,position="fill") +
